package com.simpllearn.service;

import com.simpllearn.model.Race;
import com.simpllearn.model.TolkienCharacter;
...
class DataServiceTest {
    ...
    void ensureThatInitializationOfTolkeinCharactorsWorks() {
        TolkienCharacter frado = new TolkienCharacter("Frado", 33, Race.HOBBIT);

        assertEquals(33, frado.age);
        assertEquals("Frado", frado.getName());
        assertNotEquals("Frodon", frado.getName());
    }

    @Test
    void ensureThatEqualsWorksForCharaters() {
        Object jake = new TolkienCharacter("Jake", 43, Race.HOBBIT);
        Object sameHake = jake;
        Object jakeClone = new TolkienCharacter("Jake", 12, Race.HOBBIT);

        assertEquals(jake, sameHake);
